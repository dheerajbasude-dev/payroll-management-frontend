...
export default function PayrollChart({ totals = { totalGross: 0, totalTax: 0, totalNet: 0 } }) {
      ...
      <ResponsiveContainer width="100%" height="100%">
        <PieChart>
          <Pie
            ...
            outerRadius={120}
            paddingAngle={6}
            label={(entry) => (entry.value ? `${entry.name}: ${Math.round((entry.value / (totalGross || 1)) * 100)}%` : "")}
          >
            {data.map((entry, i) => (
